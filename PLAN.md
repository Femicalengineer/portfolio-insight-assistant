...
- **Step 5** (`consult_concepts_specialist` + `consult_portfolio_analyst`) — `consult_portfolio_analyst` is **done**. Built in `012_app/agent_service/consult_portfolio_analyst.py` as a local `@tool` (`analyze_portfolio`, with a Pydantic `ArgsSchema` giving the model per-field descriptions — no MCP server involved, unlike the market trading specialist) wrapped in a `create_agent` specialist, same `hr_agent`-shaped pattern as `consult_market_trading_specialist`. Takes each asset's expected return/volatility/correlation (given/assumed inputs, not fetched — see `DECISIONS.md`) and computes the tangency (Sharpe-maximizing) portfolio weights, that portfolio's own expected return/volatility/Sharpe ratio, and the optimal risky-vs-risk-free split given a risk aversion input. Confirmed working end-to-end against a real question, numbers hand-verified correct. `consult_concepts_specialist` not yet started — groundwork done: the MPT deck's knowledge base scope was permanently narrowed to slides 3–6 and 26–48 (see intro note above and `DECISIONS.md`), extracted into a structured corpus (`012_app/agent_service/data/mpt_deck_content.json`) including written descriptions of each chart/diagram, ready for chunking/embedding.

Step 6 (`main_portfolio_agent`, top-down) not yet started. Full practical detail on everything done so far is in `RUNBOOK.md`.
