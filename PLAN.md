...
- **Step 5** (`consult_concepts_specialist` + `consult_portfolio_analyst`) — in progress. Not yet started building either specialist itself, but groundwork done: the MPT deck's knowledge base scope was permanently narrowed to slides 3–6 and 26–48 (see intro note above and `DECISIONS.md`), and that content has been extracted into a structured corpus (`012_app/agent_service/data/mpt_deck_content.json`) including written descriptions of each chart/diagram, ready for chunking/embedding. Also decided: `consult_portfolio_analyst` will use given/assumed expected-return and volatility inputs rather than fetching historical price data (see `DECISIONS.md`) — the deck's own slide 36 example (Debt 8%/12%σ, Equity 13%/20%σ, correlation 0.30, risk-free 5%) is a good candidate worked example to build/test against.
