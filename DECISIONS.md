...
### Step 6 system prompts: reviewed all four agents deliberately, not left as whatever was quick to write
- **Options considered per prompt:** accept each specialist's prompt as originally written while building it, vs. deliberately revisit each one now against the specific questions raised when this step was scaffolded (speculation guardrails, few-shot vs. zero-shot, scope tightness, where a guardrail should live)
- **Decision, `consult_market_trading_specialist`:** added an explicit two-part speculation guardrail — don't speculate using own knowledge if missing info needed to call `get_quote` correctly, *and* don't speculate if the tool's result still doesn't answer the question. **Reasoning:** this specialist is the one genuine risk case for confident-but-wrong answers — stock prices are exactly the kind of live, time-sensitive fact an LLM might answer fluently from stale training data instead of calling the tool, unlike (say) portfolio math, where there's nothing to "know" without the tool doing the actual calculation.
- **Decision, `consult_concepts_specialist`:** no change — reviewed and confirmed its existing prompt ("only answer using the documents returned from the tool... say you don't have that information" otherwise) already fully addresses the "should this be scoped to only retrieved context" question from when this step was scaffolded.
- **Decision, `consult_portfolio_analyst`:** added the same two-part speculation guardrail as the market trading specialist, plus an explicit instruction to *ask the user for missing inputs* (expected returns, volatilities, correlations, risk-free rate, risk aversion) rather than just declining. Explicitly decided **against** adding a few-shot worked example. **Reasoning for the guardrail:** same shape as market trading, but this specialist's actual default behavior (observed directly, before any prompt change) was already reasonable — asking for what it needed rather than guessing — so this makes that already-good behavior explicit and reliable rather than fixing a real observed problem. **Reasoning against few-shot:** `analyze_portfolio`'s output isn't the LLM generating a calculation by pattern-matching a format — the real math runs in Python, and the tool call's structure is already enforced by its Pydantic `ArgsSchema`. There's also direct evidence it isn't needed: this specialist produced a clear, well-formatted answer zero-shot when tested earlier, with no example to anchor it. Few-shot would add prompt complexity without a corresponding correctness or consistency benefit — the same unjustified-complexity trap already flagged elsewhere in this doc (see the Concepts specialist retrieval-depth entry).

### `main_portfolio_agent`'s system prompt: no financial-disclaimer language yet, defer entirely to step 9
- **Options considered:** add some baseline disclaimer-awareness to `main_portfolio_agent`'s prompt now (step 6), as a first line of defense alongside the post-hoc guardrail step 9 will add later; vs. leave the prompt as-is (already has the "don't speculate, say I don't know" guardrail from earlier) and defer the whole financial-disclaimer question to step 9
- **Decision:** defer entirely to step 9
- **Reasoning:** the exact trigger condition and wording for the financial-disclaimer guardrail is already sitting in this doc's "Open, not yet decided" list — meaning it was always meant to be resolved as one coherent decision (prompt placement, wording, and trigger condition together) when step 9 actually happens, not settled piecemeal now without knowing what the real guardrail will look like. Writing something into the prompt today risked being inconsistent with or redundant to whatever gets decided later, and would likely mean revisiting this file again anyway. `main_portfolio_agent`'s system prompt is otherwise considered done for step 6.
...
### `main_portfolio_agent`'s checkpointer: `InMemorySaver`, not persistent storage
- **Options considered:** (A) no checkpointer at all (each `/chat` request is stateless, no cross-turn memory); (B) `InMemorySaver` (in-process, per-request `thread_id` gives each caller its own conversation state, lost on process restart or across multiple instances); (C) a persistent checkpointer (e.g. a hosted key-value store or database) that survives restarts and scales across instances
- **Decision:** B
- **Reasoning:** Once `fastapi_app.py` needed a real `thread_id` per request (for a future Gradio frontend to hold a conversation, not just fire one-off questions), *some* checkpointer was needed for that field to mean anything — A was ruled out once that requirement was real, not hypothetical. C was skipped for the same reason the MCP server's access-token caching skipped external persistence (see that entry above): added infrastructure/cost for a portfolio project without high-traffic or multi-instance requirements. B is the same honest, documented tradeoff shape — conversation history lives only in this process's RAM, so a Render restart or scaling past one instance loses or fragments it. Revisit only if this actually becomes a practical problem once deployed.
...
### `main_portfolio_agent`'s FastAPI service: stays local (Docker), not deployed to Render
- **Options considered:** (A) deploy `portfolio-agent-service` as a second Render Web Service, per the original step 7 plan; (B) reduce the service's memory footprint further (deferring heavy imports, splitting the concepts specialist into its own separate free-tier service) until it fits Render's free-tier RAM limit; (C) pay for a higher-RAM Render tier for just this service; (D) run it locally via Docker only, and leave the MCP server as the one actually deployed piece
- **Decision:** D
- **Reasoning:** Render's free tier caps a service at 512MB RAM. `portfolio-agent-service` bundles all three specialists plus LangGraph/LangChain/FastAPI in one process, and even after deferring the heaviest imports (`torch`/`sentence-transformers`, only loaded on first real use instead of at boot — a real, kept improvement, not reverted), the service still got OOM-killed on startup (`exit 137`) on the free tier. C was ruled out — a deliberate, explicit call not to pay for additional infrastructure on a portfolio project. B (splitting the concepts specialist into its own third free Render service, so the RAM-heavy RAG pipeline isn't sharing a budget with the rest of the orchestrator) is a real, viable option and was seriously considered, but was set aside for now given the amount of time already spent chasing this specific constraint (2026-08-16) — worth revisiting later if a fully-deployed demo becomes worth the extra restructuring. D keeps the actually load-bearing parts of the architecture intact: the MCP server genuinely is deployed and public, and `portfolio-agent-service` (running locally in Docker, fully proven end-to-end — health, all three specialists, real live AAPL quotes) genuinely does reach out over the real internet to that live service, which is the real content of the "networked, multi-service architecture" demonstration. What's lost is only a public URL for the agent service itself, not the underlying technical proof. Consistent with the precedent already set for the Gradio frontend (see "Open, not yet decided" below) — not every piece of a system needs its own public deploy target to demonstrate the architecture is real.
