# Runbook — Portfolio Insight Assistant

Practical "how do I actually do X" reference for this project — the concrete commands and procedures, as opposed to `PLAN.md` (what to build, in what order) or `DECISIONS.md` (why each choice was made). Each section also notes how to sanity check that step actually worked before moving on.

---

## Questrade practice account setup

- Sign up at `questrade.com/api/free-practice-account` (a distinct signup flow from "Questrade Global," which is a separate forex/CFD product with its own unrelated practice account).
- Account type: **Margin** — no tax-advantaged distinctions matter for a practice account, and Margin has no registered-account trading restrictions.
- Generate an API refresh token: log in to the practice account → account dropdown → **API Centre** → **Register a personal app**:
  - Name and description are both mandatory
  - OAuth scopes: check both (market data + balances/positions/orders) up front, so you don't have to re-register later when adding trading
  - Callback URL: must be **HTTPS** — a placeholder like `https://example.com/callback` is fine, since the manual token flow below never actually redirects there
- On the app's page: **New manual authorization** → **Generate new token** → **Copy token** before closing the popup (Questrade never shows it again).
- The token shown is a **refresh token**, not the access token used to actually authenticate. It expires in **7 days** and is **single-use** — every exchange issues a new refresh token that replaces it.

**Sanity check:** the account dashboard should show an explicit "Practice"/"Demo" label.

## Local secrets setup

- `.env` lives at the project root (`012_capstone_portfolio_insight_assistant/.env`), not inside `012_app/mcp_server/`.
- `.gitignore` at the project root excludes it.
- Format: `QUESTRADE_REFRESH_TOKEN=<value>`.

**Sanity check, without ever printing the secret itself:** line count (`wc -l .env` should be 1), and the value's length/absence of stray quotes or whitespace — enough to rule out file corruption without exposing anything sensitive.

## OAuth token exchange (`get_access_token`)

- Endpoint: `https://login.questrade.com/oauth2/token`, with `grant_type=refresh_token` and `refresh_token=<token>` sent as **query parameters** (`params=` in `requests`), not a request body (`data=`) — despite it being a POST.
- Response JSON contains: `access_token`, `refresh_token` (the new, rotated one), `token_type`, `expires_in`, `api_server` (the host to send all subsequent API calls to — not a fixed domain).
- **Persistence across runs:** because the refresh token is single-use, the newly-issued one has to be saved back to `.env`, or the next run will use the now-dead old value.
  - `find_dotenv()` gets the actual resolved path `load_dotenv()` used (don't hardcode `.env` as a literal string — that's resolved relative to wherever the script happens to be *run from*, not where the script file lives).
  - `set_key(path, "QUESTRADE_REFRESH_TOKEN", new_value)` rewrites that file.
  - `load_dotenv(override=True)` is required (not just `load_dotenv()`) for a later call in the same process to pick up a value that changed on disk since the last load.

**Sanity check:** confirm the returned dict has all five fields (`access_token`, `refresh_token`, `token_type`, `expires_in`, `api_server`), and check `.env`'s modification time and structure after the call to confirm the rotated token actually got written back.

## `get_quote`

- Questrade quotes are keyed by an internal numeric **symbol ID**, not the ticker string — two calls needed:
  1. `GET {api_server}v1/symbols/search?prefix=<ticker>` → pull `symbols[0]["symbolId"]`
  2. `GET {api_server}v1/markets/quotes?ids=<symbolId>` → pull `quotes[0]["askPrice"]`
- Both authenticated via header: `Authorization: Bearer <access_token>` (not a query param, unlike the OAuth exchange itself).
- Return type hints matter specifically for MCP tools: FastMCP builds a schema from the function's declared return type and validates the actual return value against it at call time — `askPrice` is a `float`, so the tool's signature needs to say `-> float`.

**Sanity check:** call it directly as a plain function first (outside MCP entirely) with a real ticker and confirm a real numeric price comes back, before wrapping it as a tool.

## Wrapping as an MCP tool

- `@mcp.tool()` — the function signature becomes the schema an agent can actually call with. Anything the tool needs that an agent *can't* supply (like auth tokens) has to be fetched internally inside the function, not received as a parameter.
- Progression: `stdio` first (`mcp.run(transport="stdio")`), then `http` (`mcp.run(transport="http", port=..., host="0.0.0.0")`).
- Testing requires an actual MCP client — curl can't call `@mcp.tool()` functions, since they only speak the MCP protocol. Use a small separate script:
  ```python
  from fastmcp import Client
  client = Client("questrade_mcp_server.py")        # stdio: points at the file directly
  client = Client("http://localhost:8765/mcp")       # http: needs the /mcp path suffix
  async with client:
      result = await client.call_tool("get_quote", {"symbol": "AAPL"})
  ```

**Sanity check:** the test script above, run against a real symbol, should return `is_error=False` and a real numeric `data` value.

## `/health` endpoint

- `@mcp.custom_route(path="/health", methods=["GET"])` registers a plain HTTP route alongside the MCP protocol — reachable by anything that speaks ordinary HTTP (curl, a browser, Render's health monitor), not gated behind MCP at all.
- The handler accepts the incoming request as a parameter, and returns a Starlette `JSONResponse` (`from starlette.responses import JSONResponse`) — not a plain dict the way a `@mcp.tool()` function can.

**Sanity check:** with the server running, either `curl http://localhost:8765/health` in a second terminal, or just open `http://localhost:8765/health` directly in a browser's address bar — both work identically for a plain GET endpoint.

## Dockerizing

- `Dockerfile` pattern (same shape as 010): base image → `WORKDIR` → copy + install `requirements.txt` → copy the actual app file → `CMD`.
- `CMD` is the literal command used to start the app (`["python", "questrade_mcp_server.py"]`) — the same thing typed in a terminal to run it locally.
- `requirements.txt` only lists third-party packages actually installed via `pip` — standard-library modules (`os`, `asyncio`) don't belong in it, and the package name isn't always the same as the import name (`pip install python-dotenv`, but `from dotenv import ...`).
- Never `COPY .env` into the image — secrets get forwarded at `docker run` time instead.

**Sanity check:** `docker build -t <name> .` should complete without error before moving on to `docker run`.

## Running and testing the container

1. **Forward the secret in** — a container has no access to your local `.env` file or shell environment unless explicitly passed. Full copy-paste sequence, in one terminal, run start to finish without anything else in between (a local non-Docker run or another test in the middle will rotate the token and leave the shell's exported copy stale):
   ```bash
   # if a previous container is still running, stop and remove it first
   # (--name below means it's always called "questrade-mcp", not a random Docker-generated name)
   docker stop questrade-mcp 2>/dev/null
   docker rm questrade-mcp 2>/dev/null

   # load the current refresh token into this shell session
   cd "/Users/Sarah/Documents/Programming/MMAI Python Bootcamp/2026_Upskill/Projects/012_capstone_portfolio_insight_assistant"
   set -a
   source .env
   set +a

   # build and run
   cd 012_app/mcp_server
   docker build -t questrade-mcp-server .
   docker run --name questrade-mcp -p 8080:8765 -e QUESTRADE_REFRESH_TOKEN="$QUESTRADE_REFRESH_TOKEN" questrade-mcp-server
   ```
   In a **second terminal**, once the container's running:
   ```bash
   # plain HTTP route -- curl or a browser both work
   curl http://localhost:8080/health

   # MCP tool -- needs the actual client script, not curl
   # (test_servers.py's Client URL should point at http://localhost:8080/mcp)
   python3 test_servers.py
   ```

2. **Port mapping (`-p host:container`)** — the two sides mean different things:
   - The **right side** (container port) must match whatever the app actually binds to inside the code (`8765`).
   - The **left side** (host port) is an arbitrary number chosen for what you type on your own machine (`8080` here) — doesn't need to match anything in the code.
   - Curl/test scripts always target the **host** side.

3. **`host="0.0.0.0"` in `mcp.run(...)` is required for Docker.** `127.0.0.1` only accepts connections that originate from the same machine, and traffic Docker forwards in from outside the container doesn't count as "the same machine" from the container's point of view — `0.0.0.0` means "accept connections on any interface." Works fine for local (non-Docker) testing too, so there's no downside to always setting it.

4. **Refresh-token persistence inside a container is a known open item, not yet fully solved** (see `DECISIONS.md`): there's no `.env` file inside a container to write the rotated token back to, so `get_access_token` currently just skips that step gracefully when none exists. This means a container can successfully call `get_quote` once per token it was started with; a second call or a restart needs a fresh one. The real fix (keeping a rotating credential valid across restarts on a real deployed service) is deferred to the actual Render deploy.

## Deploying to Render

- GitHub repo: `github.com/Femicalengineer/portfolio-insight-assistant` — pushed with `git add` / `git commit` / `git remote add origin <url>` / `git push -u origin main`.
- Render dashboard → **New** → **Web Service** → connect the GitHub repo.
- **Root Directory**: `012_app/mcp_server` — the Dockerfile isn't at the repo root, so Render needs to know where to actually find it and build its context from.
- **Dockerfile Path**: `012_app/mcp_server/Dockerfile` — per Render's own field description, this one is relative to the *repo root*, not the Root Directory above (a real, easy-to-miss distinction between the two fields).
- Environment variable: `QUESTRADE_REFRESH_TOKEN` set directly in Render's dashboard (not committed anywhere in the repo) — needs to be a **currently valid** token, checked directly against Questrade before pasting it in, since a stale one fails the exact same way it does locally.
- Deployed URL: `https://portfolio-insight-assistant.onrender.com`

**Sanity check:** same two-step check as local Docker testing, just against the real URL instead of `localhost` — `curl https://portfolio-insight-assistant.onrender.com/health`, then point `test_servers.py`'s `Client(...)` at `https://portfolio-insight-assistant.onrender.com/mcp` and confirm `get_quote` still works for real.

**Sanity check, in order:** `docker ps -a` confirms the container is running and shows its actual port mapping; `curl`/browser against `/health` on the **host**-side port confirms the server is reachable at all; the MCP test script (pointed at `http://localhost:<host_port>/mcp`) confirms `get_quote` works end-to-end through the container. If something fails, `docker logs <container name>` shows the real traceback happening inside the container — often more informative than whatever error message made it back to the client.

## Building `consult_market_trading_specialist` (step 4)

- Load the deployed MCP server's tools with `MultiServerMCPClient` (from `langchain_mcp_adapters.client` — a separate package, not `langchain.mcp_adapters`), `http` transport, pointed at the live URL's `/mcp` path:
  ```python
  client = MultiServerMCPClient({
      "questrade": {"transport": "http", "url": "https://portfolio-insight-assistant.onrender.com/mcp"}
  })
  tools = await client.get_tools()  # async -- needs await
  ```
- Build the specialist with `create_agent(model=..., system_prompt=..., tools=tools)`, same as any other agent in this curriculum.
- **Don't invoke the specialist inside its own definition file.** Matches 005's `hr_agent` pattern exactly: the agent is defined once, and only ever invoked either through a separate test script or (later) through a `@tool`-wrapped function that a supervisor calls — never a standalone `.invoke()` call sitting in the same file as the `create_agent(...)` call itself.
- Since building the agent requires an `await` (loading MCP tools is async), the definition file's entry point needs to be an `async def` function that **returns** the built agent, wrapped in `if __name__ == "__main__":` — not a bare `asyncio.run(...)` at module level, which would fire as a side effect the moment the file is *imported*, not just when run directly.
- **Invoking a `create_agent` result always takes a dict matching its State schema, never a bare string:** `agent.invoke({"messages": [{"role": "user", "content": "..."}]})` (or `await agent.ainvoke(...)` in an async context). This is because `create_agent` runs on a LangGraph `MessagesState` schema (`{"messages": [...]}`, using an `add_messages` reducer that appends rather than overwrites) — not a simple one-variable LCEL chain, which is the context where invoking with a plain string works.
- Result access is dict-style, not attribute-style: `result["messages"][-1].content`, never `result.messages`.
- **Debugging a tool error inside an agent:** `create_agent` catches tool errors and feeds them back to the model as a `ToolMessage`, which the model then paraphrases vaguely ("I encountered an error..."). The real error is inside `result["messages"]` — print the full list, not just the last message, to find the actual `ToolMessage` content.
- **The deployed service has its own separate copy of `QUESTRADE_REFRESH_TOKEN`, set in Render's dashboard — updating local `.env` alone does not fix a stale token there.** Both need updating independently when the token dies (which happens more often than expected — see `DECISIONS.md`'s note on the accepted in-memory-caching tradeoff). Updating an env var through Render's dashboard UI triggers an automatic redeploy; if it doesn't, there's a manual "Deploy" button.

**Sanity check:** ask the specialist a real question (e.g. "what is the price of AAPL stock?") and confirm it correctly chooses to call `get_quote` and returns a real answer — not just that the file runs without crashing.

## Extracting the MPT deck into a RAG-ready corpus

- The `Read` tool can't open `.pptx` files directly (binary format, unsupported) — use `python-pptx` (`pip install python-pptx`) instead.
- Per slide: `slide.shapes` gives you the text frames (`shape.text_frame.text`) and pictures (`shape.shape_type == 13`, then `shape.image.blob` to get the raw image bytes to save as a file); `slide.notes_slide.notes_text_frame.text` gives the speaker notes.
- **Don't assume text/notes alone capture the real content.** For this deck specifically, most of the substantive material is in charts/diagrams (efficient frontier plots, CAL diagrams), not slide text — extracted images were reviewed directly (via the `Read` tool's image support) and given a written description, folded into the same corpus entries the text/notes live in. See `DECISIONS.md` for why this was chosen over notes-only or full multimodal retrieval.
- **Not every image is a chart worth describing** — one slide in this deck turned out to be an unrelated interactive quiz question (not a plot at all), and some "images" on a single slide are really animation fragments of one diagram, not distinct content. Worth actually looking before writing a description for each one, rather than assuming every embedded image is meaningful content.
- Final corpus is a structured list, one entry per slide (`{"slide": n, "title": ..., "text": [...], "notes": ..., "image_descriptions": [...]}`) — not a single flattened document — specifically so slide number/section metadata survives for `SelfQueryRetriever` later, per the retrieval techniques already committed to in `DECISIONS.md`.
- Output: `012_app/agent_service/data/mpt_deck_content.json`, scoped to slides 3–6 and 26–48 only (see `DECISIONS.md`'s knowledge-base-scope decision) — 27 slide entries, 10 with image descriptions.

**Sanity check:** spot-check a few entries against the actual deck — does the extracted text/notes match what's on the slide, and does the written image description actually match what the chart shows (numbers, axis labels, the specific relationship being illustrated), not just a generic "this is a chart about portfolio theory."

## Building `consult_portfolio_analyst` (step 5)

- Unlike `get_quote`, this specialist's tool (`analyze_portfolio`) is a **local `@tool`**, not MCP-sourced — no external server involved, just computation on given inputs. Same `create_agent`-wrapping-a-tool shape as `consult_market_trading_specialist`, just a local `@tool` instead of an MCP-loaded one.
- **Per-argument descriptions on a tool**, via a Pydantic `args_schema`, matter a lot once a tool has several parameters (this one has six, some structurally complex like a correlation matrix) — worth doing whenever a tool's inputs aren't self-explanatory from bare type hints alone:
  ```python
  from pydantic import BaseModel, Field
  from langchain_core.tools import tool

  class ArgsSchema(BaseModel):
      some_param: float = Field(..., description="...")

  @tool(args_schema=ArgsSchema)  # NOT arg_schema (no 's') -- and pass the class itself, not `.schema()`'s dict output
  def my_tool(some_param: float) -> dict:
      ...
  ```
- **Given/assumed inputs vs. computed outputs, a distinction worth keeping straight:** each asset's own expected return/volatility/correlation are facts about the assets (can't be derived by the tool, have to come from outside — historical data, estimates, or given inputs per the decision in `DECISIONS.md`). Portfolio *weights*, by contrast, are the decision variable this specific tool exists to solve for — supplying them as an input would be circular. Don't conflate "things that must be given" with "things the calculation produces."
- **Testing a `@tool`-decorated function directly (not through an agent) uses `.invoke({...})`, not a plain function call** — `@tool` turns the function into a LangChain tool object, so `my_tool(x=1, y=2)` doesn't work the way it did before decorating; `my_tool.invoke({"x": 1, "y": 2})` does. Same "test as a plain function first, decorate once the logic's proven" progression as `get_quote`, just with `.invoke()` as the test call once `@tool` is live, instead of reverting the decorator.
- **Validate against a real worked example with a known answer**, not just "it ran without an error" — the MPT deck's own slide 36 (Debt 8%/12%σ, Equity 13%/20%σ, correlation 0.30, risk-free 5%) has a stated final answer (~40/60 optimal risky-asset split) to check the code against directly, rather than trusting arbitrary placeholder numbers.

**Sanity check:** ask the specialist a real question with concrete numbers and confirm the tool's output matches a hand-calculated (or deck-verified) answer — not just that the agent responds fluently.

## Building `consult_concepts_specialist` (step 5, the RAG specialist)

- **Pipeline composition** (not shown pre-solved anywhere in the curriculum — 006 covers each retrieval technique individually, combining them was left as an unworked exercise): `SelfQueryRetriever` first (extracts metadata filters from the question), wrapped by `MultiQueryRetriever` as its base `retriever=` (both are `BaseRetriever` subclasses, so this composition is valid — confirmed by checking what each inherits from, not assumed), then re-ranking as a separate final step over whatever `Document` list comes back (re-ranking works on a plain list, not a retriever, so it isn't "wrapped" the same way).
- **Actual runtime order, worth being precise about:** `MultiQueryRetriever` generates its 3 alternate phrasings *first* (one LLM call), then runs the *entire* `SelfQueryRetriever` process (its own separate LLM call to extract a filter, then search) *separately for each phrasing* — not "filter once, then search 3 times within that filtered subset." Final results are a **union** (`unique_union`), not an intersection — a document only needs to show up via *one* phrasing to make the final list.
- **`MultiQueryRetriever`'s default prompt can produce a malformed "query."** Its default output parser (`LineListOutputParser`) just splits the LLM's raw response on newlines and drops empty lines — it does not filter out non-question lines. Claude's response to the default prompt included a markdown header (`# Alternative Question Versions`) before the actual 3 questions, and that header got treated as a 4th "query" and handed to the base retriever, which couldn't parse it (visible in the error text itself: the confused LLM response literally described the bad input). Fix: pass a custom `prompt=` to `MultiQueryRetriever.from_llm(...)` that explicitly forbids headers/numbering/extra text — must keep `input_variables=["question"]`, since `generate_queries()` always calls the chain with `{"question": ...}`.
- **`AttributeInfo`'s declared `type=` isn't strictly enforced** by the query constructor — a known LangChain limitation (confirmed via a real GitHub issue, not assumed). Even with `type="integer"` declared, the LLM-driven query constructor can still emit a string value for that field. Real fix has to happen elsewhere: either store the metadata as the type the constructor tends to produce (e.g. strings), or coerce the value's type in the translator (`SimpleChromaTranslator.visit_comparison`) before building the filter, since that's the layer that actually knows what type each attribute should be.
- **A retriever's `.invoke()` takes a plain string, not a dict.** `{"query": "..."}` will often *appear* to work when called directly (an LLM can still extract meaning from a stringified dict), but it's not the actual supported interface — `BaseRetriever.invoke(query: str)` is. `MultiQueryRetriever` internally calls `self.retriever.invoke(query, ...)` with a plain string, confirming this is the real contract.
- **`@tool(response_format='content_and_artifact')`** lets a tool return a `(content, artifact)` tuple — `content` is the string the model actually sees, `artifact` is the raw data (here, the reranked `Document` list) available in state without needing to be re-parsed out of a string.
- **Reading an agent's response messages uses `.type`, not `.role`.** `.role` is only a key in the *input* dict format (`{"role": "user", "content": "..."}`); the actual `HumanMessage`/`AIMessage`/`ToolMessage` objects that come back in `result["messages"]` have `.type` instead (`"human"`, `"ai"`, `"tool"`) and `.content`.

**Sanity check:** ask the agent a real conceptual question (not one with a hardcoded/obvious filter answer) and read through the full `result["messages"]` chain, not just the final answer — confirm it actually called `retrieve_results`, and that the retrieved content is genuinely relevant, not just that the agent produced fluent-sounding text.

## Embedding the MPT deck corpus into Chroma (step 5, concepts specialist groundwork)

- Reuses 003's exact pattern: `HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")` (**keyword argument only** — passing the model name positionally raises `TypeError: takes 1 positional argument but 2 were given`), `Document(page_content=..., metadata={...})` objects, `Chroma.from_documents(documents=..., embedding=..., collection_name=..., ids=[...])`.
- **`page_content` needs to be joined plain text, not `str()` on a list.** The corpus's `text` and `image_descriptions` fields are `list[str]` — calling `str(some_list)` embeds the literal Python repr (brackets, quotes, commas) as text. Use `" ".join(...)` instead, so the embedding sees natural language.
- **`.persist()` is deprecated** (Chroma 0.4.x+ auto-persists) — the thing that actually matters is passing `persist_directory=...` directly to `Chroma.from_documents(...)`. Without it, the store is in-memory only regardless of any `.persist()` call afterward.
- A `None` value in Chroma document metadata (some slides have `title: None`, since they have no text frames) turned out not to cause any issue — confirmed by actually running it, not assumed.
- Output: `012_app/agent_service/data/chroma_mpt_deck_vectorstore/` (persisted Chroma collection, 27 documents, one per in-scope slide, metadata: `slide`, `title`, `section`).

**Sanity check:** confirm the persist directory actually contains a real `chroma.sqlite3` file with a nonzero size after running — "no errors printed" isn't proof anything was actually written to disk.

## Building `main_portfolio_agent` (step 6, the top-level orchestrator)

- **Subagents pattern, same shape as 005's `hr_agent` → `consult_hr_agent`:** each specialist agent gets built once, then wrapped in a small `@tool` function that calls `.invoke(...)` (or `.ainvoke(...)`) on it and returns just `result["messages"][-1].content` — `main_portfolio_agent` never sees a specialist's internal state, only whatever that wrapper explicitly returns.
- **Calling `.invoke()`/`.ainvoke()` directly on an imported module is a real mistake to watch for** — you need the actual agent *instance* (what that module's `main()` returns), not the module object itself. `some_module.invoke(...)` will fail with `AttributeError: module '...' has no attribute 'invoke'`; it needs to be `some_module.main().invoke(...)`.
- **One async tool poisons the whole call chain with async.** `get_quote` is only ever async (loaded via `MultiServerMCPClient`), so `consult_market_trading_specialist_agent` (the tool wrapping that specialist) has to be `async def` too, awaiting its way down. That in turn means the *whole* `main_agent` has to be invoked via `asyncio.run(main_agent.ainvoke(...))`, not plain `.invoke()` — even though the other two specialists' tools are perfectly ordinary sync functions. LangGraph supports mixing sync and async tools under one agent, but only if the top-level invocation itself is async.
- **Don't call a sync-wrapped async function (`def main(): return asyncio.run(get_agent())`) from inside code that's already running in an event loop.** `asyncio.run()` cannot start a second, nested event loop — doing so raises `RuntimeError: asyncio.run() cannot be called from a running event loop`. Inside an async tool (which *is* already running inside `main_agent`'s own event loop), skip the sync wrapper and call the raw async function directly with `await` instead (e.g. `await (await consult_market_trading_specialist.get_agent()).ainvoke(...)`, not `consult_market_trading_specialist.main().ainvoke(...)`).
- **System prompts got a deliberate second pass here, not just accepted as first-written.** Concretely: an explicit two-part speculation guardrail (don't guess before calling the tool if inputs are missing; don't guess after calling it if the result still doesn't answer the question) mattered most for the market trading specialist (stock prices are exactly the kind of live fact an LLM can confidently get wrong from stale training data) and was added to the portfolio analyst too, even though its default behavior was already reasonable. Few-shot examples were explicitly considered and rejected for the portfolio analyst — its output isn't the model pattern-matching a format, the real math runs in code and the tool call is already schema-validated. See `DECISIONS.md` for the full reasoning per prompt.

**Sanity check:** ask `main_agent` a single question that requires all three specialists at once (a live quote, a calculation needing given inputs, and a conceptual question) and confirm the combined answer is actually correct — check the specific numbers by hand, not just that the response reads fluently.

## Building `fastapi_app.py` + `Dockerfile` for `main_portfolio_agent` (step 7)

- **`main_portfolio_agent.py` needed a `build_main_agent()` factory, not a module-level agent.** Same reason as `consult_market_trading_specialist`'s `get_agent()`: `fastapi_app.py` needs to build the agent once, at import time, and hold onto that one instance across all requests (so its `InMemorySaver` checkpointer's state actually persists between calls) — a plain factory function called once at FastAPI import time does this cleanly.
- **`thread_id` needs a real checkpointer to mean anything.** Adding `checkpointer = InMemorySaver()` (`from langgraph.checkpoint.memory import InMemorySaver`) to `create_agent(...)`, then building `config = {"configurable": {"thread_id": request.thread_id}}` **fresh inside the `/chat` endpoint** (not once at module level) is what makes each caller's `thread_id` key into its own separate conversation. Building `config` at module level instead is a real bug — every request would silently share one hardcoded thread.
- **A sync FastAPI endpoint can safely call `asyncio.run(agent.ainvoke(...))`.** This differs from the `main_portfolio_agent.py`-internal rule ("don't call `asyncio.run()` from inside a running event loop"): FastAPI runs plain `def` (sync) path operations in a threadpool worker thread, which has no event loop of its own, so `asyncio.run()` there doesn't hit the nested-loop error. (An `async def` endpoint using a bare `await` instead is equally valid and slightly more idiomatic — either works.)
- **Dockerfile must `COPY` every file the entry point transitively imports, not just the entry point itself.** `fastapi_app.py` imports `main_portfolio_agent`, which imports all three `consult_*.py` specialist files — missing any of them is a `ModuleNotFoundError` at container startup. `consult_concepts_specialist.py` additionally needs the **persisted Chroma vectorstore directory** (`data/chroma_mpt_deck_vectorstore/`, a directory `COPY`, not a file) on disk at runtime via its relative-path `load_vectorstore()` call — easy to miss since it's a data dependency, not an import.
- **A package installed in your local venv can be silently missing from `requirements.txt`.** `SelfQueryRetriever` needs `lark` to parse structured queries, but it was already present in the local dev venv (installed incidentally at some earlier point) and never added to `requirements.txt` — worked locally the whole time, then failed inside a fresh Docker container with `ImportError: Cannot import lark, please install it with 'pip install lark'`. The actual traceback only showed up in `docker logs`, not in the `/chat` response itself — LangGraph's tool-node error handling catches the exception and hands the model a vague paraphrased error message instead (same "check `docker logs`/`result['messages']` for the real error" lesson as step 6, just now relevant for a deployed/containerized failure too).
- **A Docker image does not survive a host reboot as reliably as you'd expect.** After a Mac restart mid-session, `docker images` no longer listed a previously-built image (`portfolio-agent-service`), even though two older, unrelated images survived fine — likely fallout from Docker Desktop's storage having been in a bad state from an earlier disk-full crash. No fix needed beyond just rebuilding; flagging so a missing image after a restart isn't mistaken for something more mysterious.
- **`curl -d '...'` breaks on an apostrophe in the message.** A single-quoted shell string can't contain a literal `'` — `what's` in a test message ends the quoted string early, and the shell drops into a `dquote>`/`heredoc>` continuation prompt waiting for a match that'll never come (Ctrl+C to escape it). Most robust fix: write the JSON body to a file and use `curl -d @path/to/file.json`, which sidesteps shell quoting entirely regardless of what's in the message.
- **Refresh-token single-use rotation makes local and deployed environments fight over the same token if not handled carefully.** Exchanging the refresh token *anywhere* (a local `get_access_token()` call, or Render's server doing it internally) issues a new one and invalidates the old — so if you exchange it locally (e.g. testing `test_servers.py`/`questrade_mcp_server.py` directly) *after* copying a token into Render's dashboard but *before* Render's service has used it, Render's copy is now silently dead. The safe order: get a fresh token, immediately paste it into Render's dashboard **first**, confirm Render's redeploy has picked it up (wait ~1-3 min, or check via `test_servers.py` pointed at the live URL, which itself doesn't consume the local `.env` copy), and only *then* do any local exchanges if you also need local dev to keep working — or accept that local and deployed will keep needing independent re-syncs, which is the current accepted state (see `DECISIONS.md`'s access-token-caching entry).
- **`get_quote` needs a null-price fallback.** Questrade's `askPrice` field is only populated during active trading — outside market hours (a weekend, for instance) it comes back `null`, which fails FastMCP's `-> float` return-type validation (`None is not of type 'number'`). Fix: if `askPrice` is `None`, fall back to `quotes[0]['lastTradePrice']` instead. On a demo/practice account, even that can occasionally be `null` too — a still-open edge case worth remembering if `place_order` (step 8) needs to reason about "is there a valid current price" at all.
- **A `KeyError` on a dict key that should obviously be there (e.g. `response["symbols"]`) usually means the API returned an error body instead of the expected shape, not that the code is wrong.** Confirmed by printing the raw response before parsing it (`print(f"...: {response}")`, checked via Render's Logs tab) — the fastest way to distinguish "my code has a bug" from "the upstream API rejected this specific request" (auth failure, rate limiting, etc.) is to look at the literal response body, not to keep re-reading the parsing code.
- **Render's "Root Directory" and "Dockerfile Path" fields don't resolve the same way for every service — verify empirically, don't assume the MCP server's working values generalize.** For the MCP server, Dockerfile Path = `012_app/mcp_server/Dockerfile` (looks relative to the repo root) works correctly. For `portfolio-agent-service`, the exact same pattern (`012_app/agent_service/Dockerfile`) failed with Render trying to resolve a doubled path (`.../012_app/agent_service/012_app`) — it only worked once Dockerfile Path was changed to just `Dockerfile` (relative to Root Directory instead). Never fully root-caused why the two services behaved differently; if this trips again, try both forms rather than assuming which one is "correct."
- **A file excluded by `.gitignore` will build fine locally but silently break a Render deploy.** `docker build` run locally uses whatever's actually on disk, regardless of git tracking status — but Render clones fresh from GitHub, so anything gitignored (or just never committed) simply isn't there, and every `COPY` referencing it fails with "not found." The persisted Chroma vectorstore (`012_app/agent_service/data/chroma_mpt_deck_vectorstore/`) was gitignored as a generated artifact; since it's small (27 slides) and there's no build step that regenerates it inside the Dockerfile, the fix was to just remove it from `.gitignore` and commit the actual `.sqlite3`/`.bin` files rather than add a build-time regeneration step.

## Reducing deployed memory footprint: defer heavy imports (step 7)

- **Render's free tier caps a service at 512MB RAM**, and `portfolio-agent-service` got OOM-killed on boot (`Exited with status 137`, preceded by "No open ports detected" since the process never got far enough to start Uvicorn) before it ever handled a request.
- **Just *importing* `torch`/`sentence-transformers` at module level cost ~250MB+ by itself**, confirmed by checking `docker stats <container> --no-stream` immediately after a fresh `docker restart` (before any request) — 376MB baseline, uncomfortably close to the 512MB ceiling with zero headroom for FastAPI/LangGraph overhead or an actual request.
- **Fix: move the heavy imports from module level into the functions that actually use them** — `from langchain_huggingface import HuggingFaceEmbeddings` moved inside `load_vectorstore()`, `from sentence_transformers import CrossEncoder` moved inside `retrieve_results()`. This defers the memory cost to first real use of the concepts specialist instead of paying it at every container boot. A genuine, kept improvement — but on its own it **wasn't enough** to fit `portfolio-agent-service` (all three specialists + LangGraph + FastAPI in one process) under 512MB; the rest of the dependency stack alone was still enough to OOM. See `DECISIONS.md`'s entry on why the service ended up staying local instead of chasing this further.

## Running `portfolio-agent-service` locally (the actual deployment shape for step 7)

Since the agent service isn't deployed to Render (see `DECISIONS.md`), this is the real way it runs — locally in Docker, reaching out to the *deployed* MCP server over the real network.

```bash
cd "012_capstone_portfolio_insight_assistant/012_app/agent_service"
docker build -t portfolio-agent-service .

# forward the secret in, same pattern as the MCP server
cd "012_capstone_portfolio_insight_assistant"
set -a; source .env; set +a
docker stop portfolio-agent 2>/dev/null; docker rm portfolio-agent 2>/dev/null
docker run -d --name portfolio-agent -p 8080:8000 -e ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" portfolio-agent-service
```

- Container binds to port `8000` internally (`fastapi run`'s default); `-p 8080:8000` maps that to host port `8080` — curl/test requests always target the **host** side (`8080`), same host-vs-container distinction as the MCP server's own port mapping.
- Test: `curl http://localhost:8080/health`, then `curl -X POST http://localhost:8080/chat -H "Content-Type: application/json" -d @path/to/payload.json` (payload-file approach avoids shell-quoting issues, see the `curl -d '...'` apostrophe gotcha above).
- **Sanity check:** ask a question that specifically routes through `consult_market_trading_specialist_agent` (a live price question), not just the other two specialists — that's the one async/MCP-dependent path, and the only one that proves the container is genuinely reaching the deployed MCP server over the network rather than just answering from local computation/RAG.

## Disk space crises: Docker build cache and iCloud file eviction

Two real, disruptive incidents worth recognizing quickly if they happen again — both are environment/OS issues, not bugs in this project's code.

- **Repeated Docker builds of a large image silently consume tens of GB.** `portfolio-agent-service`'s image is ~9GB (bundles `torch`/`sentence-transformers`) — rebuilding it five-plus times in one session (each code tweak) can leave 25-30GB of orphaned build-cache layers behind, since Docker doesn't clean these up automatically. This is what actually caused two separate disk-full crashes in one evening, not anything related to the app itself. Fix: `docker builder prune -af` (build cache) and `docker system prune -f` (stopped containers/unused resources) — safe, everything's rebuildable. Check `docker system df` beforehand for size confirmation if the daemon is responsive enough to answer (under severe disk pressure, even simple docker commands can hang or time out).
- **A disk-full crash can leave Docker Desktop's backend processes stuck, blocking a clean relaunch.** After the daemon crashes, `open -a Docker` can silently no-op if macOS thinks Docker's "already running" because of a lingering `com.docker.backend` process. Check `ps aux | grep -i docker`, `kill -9` the specific stuck PID(s), *then* `open -a Docker`.
- **When disk space gets critically low, macOS can evict local copies of iCloud-synced files** (anything under iCloud Drive Desktop/Documents sync, which includes this whole project since it lives under `~/Documents/...`) to free space, replacing them with cloud-only placeholders. Symptoms are confusing and don't look like a storage issue at first: files `ls -la` normally (correct size/name) but time out on actual read; `git status`/`git diff` fail with `fatal: not a git repository` if `.git/HEAD` itself got evicted, even though `.git/` visibly exists. **Diagnostic:** `ls -lO <file>` — an evicted file shows `dataless` in the flags column; `du -sh <file>` shows `0B` despite `ls -la` reporting a normal size. **Fix that actually worked:** a full computer restart. Finder's "Download Now" was unreliable and sometimes offered no download option at all even after relaunching Finder or restarting the `bird` sync daemon (`killall bird`); CLI/API approaches (`brctl download`, PyObjC's `NSFileManager`) failed with misleading "file doesn't exist" errors, almost certainly a TCC permission wall restricting these download-trigger APIs to first-party apps (Finder) rather than arbitrary terminal processes. Don't burn time on CLI workarounds for this — go straight to a restart.
